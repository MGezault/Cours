...
def Serpent(nom_joueur:str, num_joueur:int,points:int=0,positions:list=None,tps_s:int=0,tps_p:int=0,tps_m:int=0,direction:str='N')->dict: # 
    ...
    Returns:
        dict: un dictionnaire contenant les informations du serpent
    """    
    serpent = {"nom_joueur" : nom_joueur ,
...
def get_num_joueur(serpent:dict)->int:
    ...
    Returns:
        int: le numéro du joueur associé à ce serpenteturn queue[-1]
    """   
    return serpent.get("num_joueur")
...
def get_queue(serpent:dict)->[int,int]: 
    ...
    """    
    liste_pos = serpent['positions'] 
    return list(liste_pos[-1])
...
def get_derniere_direction(serpent:dict)->str: 
    ...
    Returns:
        str: un des cara["points"]ctère N S E O
    """    
    return serpent.get("direction")
...
def to_str(serpent:dict)->str: #  
    ...
    """    
    res = ""
    res += str(serpent['nom_joueur']) + " " + "->" + " " + str(serpent['points']) + " " + "s:" + str(serpent['tps_s']) + " " + "m:" + str(serpent['tps_m']) + " " + "p:" + str(serpent['tps_p'])
    return res
...
def maj_temps(serpent:dict): 
    ...
    Args:
        serpent (dict): le serpent considéré
    """
    for bonus in ["tps_s","tps_p","tps_m"]:
        if serpent.get(bonus) >= 1:
            serpent[bonus] -= 1
...
def serpent_2_str(serpent: dict, sep=";") -> str:  
    ...
    la deuxième ligne donne la liste des positions du serpent en commençant par la tête
    ...
    res = str(serpent['nom_joueur']) + sep + str(serpent['num_joueur']) + sep + str(serpent['points']) + sep + str(serpent['tps_s']) + sep + str(serpent['tps_m']) + sep + str(serpent['tps_p']) + sep + str(serpent['direction'])
    positions = serpent['positions']
    if positions is None:
        positions = []
    else:
        res += ' \n'
    position_str = ""
    for lgn, col in positions:
        position_str += str(lgn) + sep + str(col) + sep
    res += position_str.rstrip(sep)
    res += "\n"
    return res
...
def serpent_from_str(la_chaine, sep=";")->dict: 
    ...
    """  
    res = dict()
    information_split = la_chaine.split(sep)
    information_rangee = []
    i = 0
    for info in information_split:
        if "\n" in info:
            information_rangee = information_rangee + (info.replace("\n", "").split())
        else:
            information_rangee.append(info)
    for clé in ['nom_joueur', 'num_joueur', 'points', 'tps_s', 'tps_m', 'tps_p', 'direction']:
        if clé == 'nom_joueur' or clé == 'direction':
            res[clé] = information_rangee[i]
        else:
            res[clé] = int(information_rangee[i])
        i += 1


    i2 = 7
    position = []
    while i2 < len(information_rangee) - 1:
        position.append((int(information_rangee[i2]), int(information_rangee[i2 + 1])))
        i2 += 2
...
def copy_serpent(serpent:dict)->dict:
    ...
    """ 
    return {
    "nom_joueur" : get_nom(serpent),
    "num_joueur" : get_num_joueur(serpent),
    "points" : get_points(serpent),
    "positions" : (get_liste_pos(serpent)).copy(),
    "direction" : get_derniere_direction(serpent),
    "tps_m" :get_temps_mange_mur(serpent),
    "tps_p" : get_temps_protection(serpent),
    "tps_s" : get_temps_surpuissance(serpent),
    }
